...
function createDescription(leadData) {
  ...
  const descriptionFields = [
    ...
    { label: "This package doesn't include flight, do you need assistance with flight bookings?", value: leadData[13] || "" },       // N
    { label: "How many travelers are joining? (adults & kids)", value: leadData[14] || "" }, // O
  ];
...
async function processCustomerAndLead(connection, leadData) {
  ...
  try {
    ...
    const form_name = leadData[9] || "";      // J
    const platform = leadData[11] || "";      // L
    const full_name = leadData[15] || "";     // P
    const phone_number = leadData[16] || "";  // Q
    const email = leadData[17] || "";         // R
    const city = leadData[18] || "";          // S
    const phone_number_verified = leadData[19] || ""; // T
    const lead_status = leadData[20] || "";   // U
